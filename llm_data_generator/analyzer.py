...
- **差异驱动（最重要）**：重点描述各表之间同一字段的**可能的**格式差异（这是实体匹配的核心挑战）。表间差异包括但不限于：拼写变体、缩写/截断、格式编码不同、信息嵌入方式不同、字段缺失策略不同等。大小写差异只是其中一种次要维度，不能作为唯一的表间差异
- **差异随机分布（极其重要）**：表间差异不是在所有列上同时固定出现，而是对每个实体组随机选择部分列（2-3列）体现差异，其余列在不同表之间可以格式相同。不同实体组差异出现的列应有所不同，使差异分布多样化。规范中应描述每列**可能出现的**差异类型，而不是规定每列**必须**出现差异
...
**重要原则**：以下差异维度是各列**可能出现的**差异类型。对每个实体组，随机选择 2-3 个列体现差异，其余列在不同表之间可以保持相同格式。不同实体组差异出现的列应有所不同。
...
- **差异随机分布（核心维度）**：每个实体组只需在随机 2-3 个列上体现表间差异，其余列可以格式相同。不同实体组的差异列应不同，使整体差异分布多样化。禁止每个实体组在所有列上都固定使用相同的差异模式
- **大小写差异**：同一实体的不同 variant 之间必须体现大小写的变化（如首字母大写 vs 全大写 vs 全小写 vs 混合大小写）
- **非空约束**：每张表的规范中必须注明任何表的一个实体中最多只能有 1 个空字段
...
3. 必须包含 **表间差异模式** 章节，列出所有可能的系统性跨表差异。**关键要求：差异不是在固定列上，而是随机分布在多个列上**。每个实体组随机选择 2-3 个列体现差异，其余列在不同表之间格式相同。规范中应描述每列可能出现的差异类型，而不是规定每列必须出现差异
...
12. **差异随机分布（核心要求）**：必须在规范中明确要求每个实体组只在随机 2-3 个列上体现表间差异，其余列格式相同。不同实体组差异的列应不同。禁止所有实体组在同样的列上使用同样的固定差异模式
