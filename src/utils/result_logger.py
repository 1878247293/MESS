...
A single run of the main pipeline collects each stage's inputs/outputs + metrics, then writes one JSON and one human-readable TXT.
"""
import json
import time
import numpy as np
...
class ResultLogger:
    def __init__(self, dataset_name: str, output_dir: str = "results"):
        ...
        # the level currently being recorded (the merge stage switches level by level)
        self.current_hierarchy_level: Optional[HierarchyLevel] = None

        # miscellaneous: extra data such as LLM Selecting
    ...
    def log_data_loading(self, num_tables: int, table_sizes: List[int], time: float,
        ...
        # save the first N rows of each table, so they can be inspected directly after writing
        if tables_df is not None:
            for idx, table_df in enumerate(tables_df):
                sample_records = []
                for i in range(min(sample_size, len(table_df))):
                    record = table_df.iloc[i].to_dict()
                    # numpy types cannot be serialized to JSON directly
    ...
    def _convert_to_python_type(val):
        """Downcast numpy scalars to native Python types for json.dump"""
        ...
        if all_embeddings is not None and table_sentences is not None:
            ...
            for i in range(min(sample_size, len(all_embeddings))):
                self.encoding.embedding_samples.append({
                    ...
                    'embedding_vector': all_embeddings[i].tolist()[:10],  # keep only the first 10 dims
    ...
    def _compute_tuple_stats(tuples: List[Tuple]):
        """size distribution, largest group, average group size"""
        if not tuples:
            return {}, 0, 0.0
    ...
    def log_pruning(self, prediction_before: List[Tuple], prediction_after: List[Tuple], time: float):
        """Pass the full tuple lists before/after pruning; this also computes distributions and diff samples"""
        ...
        # use the sorted tuple as the key for comparison
        ...
        # record at most 5 samples of each type
    ...
    def _save_json(self, filepath: Path):
        ...
        # recursively replace numpy types with native Python ones, otherwise json.dump will fail
    ...
    def _save_txt(self, filepath: Path):
        ...
        lines.append("=" * 80)
        lines.append(f"Run intermediate results")
        ...
        # attribute selection
        lines.append("[Attribute selection stage]")
        lines.append("-" * 80)
        if self.attribute_selection.from_cache:
            lines.append("  status: cache hit")
        else:
            lines.append("  status: actually run")
        lines.append(f"  time: {self.attribute_selection.time:.4f} s")
        lines.append(f"  selected_attrs: {self.attribute_selection.selected_attrs}")
        lines.append("  per-column sim:")
        ...
        if resource_usage:
            lines.append("[Per-stage resource stats]")
            lines.append("-" * 80)
            for stage_name, info in resource_usage.items():
                lines.append(f"  {stage_name}:")
                lines.append(f"    total time: {info.get('total_time', 0.0):.4f} s")
                lines.append(f"    peak memory: {info.get('peak_memory_mb', 0.0):.2f} MB")
                lines.append(f"    peak GPU memory: {info.get('peak_gpu_memory_mb', 0.0):.2f} MB")
                lines.append(f"    peak bytes: {info.get('peak_memory_bytes', 0)}")
                lines.append(f"    peak GPU bytes: {info.get('peak_gpu_memory_bytes', 0)}")
                lines.append(f"    sample count: {info.get('invocations', 0)}")
                lines.append(f"    memory sampling available: {info.get('available', False)}")
                lines.append(f"    GPU sampling available: {info.get('gpu_available', False)}")
            lines.append("")
        ...
        if run_summary:
            ...
            for phase_name, phase_time in run_summary.get("phase_times", {}).items():
                lines.append(f"  {phase_name}: {phase_time:.4f} s")
            lines.append(f"  total time: {run_summary.get('total_time', 0.0):.4f} s")
            lines.append(f"  peak memory: {run_summary.get('peak_memory_human', 'N/A')}")
            lines.append(f"  peak GPU memory: {run_summary.get('peak_gpu_memory_human', 'N/A')}")
            lines.append("")

        # data loading
        lines.append("[Data loading stage]")
        lines.append("-" * 80)
        lines.append(f"  time: {self.data_loading.time:.4f} s")
        lines.append(f"  number of tables: {self.data_loading.num_tables}")
        lines.append(f"  total entities: {self.data_loading.total_entities}")
        lines.append(f"  per-table sizes: {self.data_loading.table_sizes}")

        if self.data_loading.table_samples:
            lines.append("")
            lines.append("  [Table data samples]")
            for sample in self.data_loading.table_samples:
                lines.append(f"    table {sample['table_id']}:")
                lines.append(f"      columns: {sample['columns']}")
                lines.append(f"      samples (first 5):")
                for i, record in enumerate(sample['sample_records']):
                    lines.append(f"        record {i+1}: {record}")
        lines.append("")

        # encoding
        lines.append("[Encoding stage]")
        lines.append("-" * 80)
        lines.append(f"  time: {self.encoding.time:.4f} s")
        lines.append(f"  model: {self.encoding.model_name}")
        lines.append(f"  embedding shape: {self.encoding.embedding_shape}")

        if self.encoding.embedding_samples:
            lines.append("")
            lines.append("  [Encoding samples] (first 5)")
            for sample in self.encoding.embedding_samples:
                lines.append(f"    entity {sample['entity_id']}:")
                lines.append(f"      text: {sample['text']}")
                lines.append(f"      embedding vector (first 10 dims): {[f'{v:.4f}' for v in sample['embedding_vector']]}")
                lines.append(f"      vector norm: {sample['embedding_norm']:.4f}")
        lines.append("")

        # contrastive learning
        if self.contrastive_learning.enabled:
            lines.append("[Contrastive learning stage]")
            lines.append("-" * 80)
            lines.append(f"  time: {self.contrastive_learning.time:.4f} s")
            lines.append(f"  epochs: {self.contrastive_learning.epochs}")
            lines.append("")

        # merging
        lines.append("[Merging stage]")
        lines.append("-" * 80)
        lines.append(f"  total time: {self.merging.total_time:.4f} s")
        lines.append(f"  final number of tuples: {self.merging.final_tuples}")
        lines.append(f"  per-level details:")
        for level_info in self.merging.hierarchy_levels:
            lines.append(f"    Level {level_info.level}: {level_info.num_tables} tables")
            if level_info.num_tuples_before > 0:
                lines.append(f"      tuples before merge: {level_info.num_tuples_before}")
            if level_info.num_tuples_after > 0:
                lines.append(f"      tuples after merge: {level_info.num_tuples_after}")
                if level_info.num_tuples_before > 0:
                    ...
                    lines.append(f"      tuple reduction: {reduction} ({reduction_rate:.2f}%)")
            for merge in level_info.merges:
                lines.append(f"      - merge {merge.tables[0]} & {merge.tables[1]}: "
                           f"{merge.pairs} pairs, {merge.time:.4f}s")
                if merge.merge_examples:
                    lines.append(f"        examples:")
                    for ex in merge.merge_examples:
                        dist_str = f", dist={ex['distance']:.4f}" if ex['distance'] is not None else ""
                        lines.append(f"          entity {ex['entity_i']} <-> entity {ex['entity_j']}{dist_str}")
        lines.append("")

        # evaluation after merge
        lines.append("[Evaluation after merge]")
        lines.append("-" * 80)
        self._format_evaluation(lines, self.evaluation_after_merge)
        lines.append("")

        # pruning
        lines.append("[Pruning stage]")
        lines.append("-" * 80)
        lines.append(f"  time: {self.pruning.time:.4f} s")
        lines.append(f"  tuples before pruning: {self.pruning.tuples_before}")
        lines.append(f"  tuples after pruning: {self.pruning.tuples_after}")
        lines.append(f"  tuples removed: {self.pruning.tuples_removed}")
        lines.append(f"  entities before pruning: {self.pruning.entities_before}")
        lines.append(f"  entities after pruning: {self.pruning.entities_after}")
        lines.append(f"  entities removed: {self.pruning.entities_removed}")
        lines.append(f"  removal rate: {self.pruning.removal_rate:.2f}%")
        lines.append("")

        if self.pruning.tuple_size_dist_before:
            lines.append("  tuple size distribution:")
            lines.append(f"    before pruning:")
            lines.append(f"      average: {self.pruning.avg_tuple_size_before:.2f}")
            lines.append(f"      max: {self.pruning.largest_tuple_before}")
            ...
            lines.append(f"    after pruning:")
            lines.append(f"      average: {self.pruning.avg_tuple_size_after:.2f}")
            lines.append(f"      max: {self.pruning.largest_tuple_after}")
            lines.append(f"      distribution:")
        ...
        lines.append(f"  F1 improvement: {f1_improvement:+.2f}%")
        lines.append(f"  Pair-F1 improvement: {pair_f1_improvement:+.2f}%")
        lines.append("")
    ...
    def _format_evaluation(self, lines: List[str], eval_result: EvaluationResult):
        lines.append(f"  number of ground-truth tuples: {eval_result.num_ground_truth}")
        lines.append(f"  number of predicted tuples: {eval_result.num_prediction}")
